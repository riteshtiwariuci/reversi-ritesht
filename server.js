...
io.on('connection', (socket) => {
    ...
    */
    ...
    socket.on('join_room', (payload) => {
        ...
        /* Check if the data from the client is good*/
        ...
        io.in(room).fetchSockets().then((sockets) => {
            serverLog('There are ' + sockets.length + ' client(s) in the room, ' + room);
            ...
            /* Socket did join the room */
                ...
                for (const member of sockets) {
                    ...
                    /* Tell everyone that a new user has joined the chat room. SocketID not required. */
    ...
    socket.on('uninvite', (payload) => {
        ...
        io.in(room).allSockets().then((sockets) => {
            ...
            /* Uninvitee is in the room */
                ...
                socket.emit('uninvited', response);
                serverLog('uninvite command sent to xxxxx', JSON.stringify(response));
                response = {
                    result: 'success',
                    socket_id: socket.id
                }
    ...
    socket.on('game_start', (payload) => {
        ...
        if ((typeof room == 'undefined') || (room === null) || (room === "")) {
            response = {
                result: 'fail',
                message: 'the user that was engaged to play is no longer in a room'
            }
            socket.emit('game_start_response', response);
            serverLog('game_start command failed', JSON.stringify(response));
            return;
        ...
        io.in(room).allSockets().then((sockets) => {
            ...
            if ((typeof sockets == 'undefined') || (sockets === null) || !sockets.has(requested_user)) {
                response = {
                    result: 'fail',
                    message: 'the user that was enagaged to play is no longer in the room'
                }
                socket.emit('game_start_response', response);
                serverLog('game_start command failed', JSON.stringify(response));
                return;
            }
            /* Enagged player is in the room */
            else {
    ...
    /* expected payload:
    {
        'rooom' : the room to which message should be sent,
    ...
    {
        'result': 'success',
        'username': the user that sent the messgae,
    ...
    */
    ...
    socket.on('send_chat_message', (payload) => {
        ...
        /* Check if the data from the client */
        if ((typeof payload == 'undefined') || (payload === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'client did not send the payload';
    ...
    socket.on('play_token', (payload) => {
        ...
        socket.emit('play_toke_response', response);
